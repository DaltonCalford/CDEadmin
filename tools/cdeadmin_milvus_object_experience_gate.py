...
"""Gate Milvus vector object experiences."""
...
def provider_catalog(live_evidence_paths=()):
    adapter = MilvusClientAdapter.__new__(MilvusClientAdapter)
    catalog = enrich_engine_experience(
        adapter.visual_admin_catalog(catalog_for_engine('milvus'))
    )
...
def audit(live_evidence_paths=()):
    ...
    return {
        'schema': 'cdeadmin.milvus-object-gate.v1',
        'gate': 'milvus-vector-object-experience',
