...
"""Gate MongoDB document navigator and object-editor declarations."""
...
def provider_catalog(live_evidence_paths=()):
    """Return the exact provider-adapted catalog plus admitted evidence."""
    adapter = MongoDBClient.__new__(MongoDBClient)
    catalog = enrich_engine_experience(
        adapter.visual_admin_catalog(catalog_for_engine('mongodb'))
    )
...
def audit(live_evidence_paths=()):
    ...
    return {
        'schema': 'cdeadmin.mongodb-object-gate.v1',
        'gate': 'mongodb-document-object-experience',
