...
"""Qualify Dolt object editors in an isolated exact-version container."""
...
def _published_port(value):
    for line in (item.strip() for item in value.splitlines()):
        ...
        if 0 < port < 65536:
            return port
    raise RuntimeError('container did not publish a usable Dolt SQL port')
...
def _wait_until_ready(container, timeout):
    ...
    while time.monotonic() < deadline:
        ...
        if state.returncode or state.stdout.strip() != 'true':
            raise RuntimeError(
                'Dolt qualification container stopped during startup'
            )
        if port is None:
            published = _run(
        ...
        if port is not None:
            try:
                ...
                pass
        time.sleep(1)
    raise RuntimeError('Dolt qualification container did not become ready')
...
def run(image, server_log, startup_timeout=300):
    ...
    container = f'cdeadmin-dolt-object-{secrets.token_hex(6)}'
    started = False
    result = None
    try:
        _run([
            'docker', 'run', '-d', '--rm', '--name', container,
            '-e', 'DOLT_ROOT_HOST=%', '-p', f'127.0.0.1::{PORT}', image,
        ])
        started = True
        port = _wait_until_ready(container, startup_timeout)
        result = verify('dolt', '127.0.0.1', port)
    finally:
