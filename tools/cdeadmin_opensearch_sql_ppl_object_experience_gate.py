...
"""Gate OpenSearch SQL/PPL composed search object administration."""
...
def provider_catalog(live_evidence_paths=()):
    adapter = OpenSearchSQLPPLClient.__new__(OpenSearchSQLPPLClient)
    adapter.search_admin = OpenSearchClient.__new__(OpenSearchClient)
    catalog = enrich_engine_experience(
        adapter.visual_admin_catalog(
...
def audit(live_evidence_paths=()):
    ...
    return {
        'schema': 'cdeadmin.opensearch-sql-ppl-object-gate.v1',
        'gate': 'opensearch-sql-ppl-search-object-experience',
