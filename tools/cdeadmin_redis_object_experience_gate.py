...
"""Gate Redis key/value navigator and object-editor declarations."""
...
from pgadmin.cdeadmin.providers.redis.client import RedisClient  # noqa: E402
...
def provider_catalog(live_evidence_paths=()):
    adapter = RedisClient.__new__(RedisClient)
    catalog = enrich_engine_experience(
        adapter.visual_admin_catalog(catalog_for_engine('redis'))
    )
...
def audit(live_evidence_paths=()):
    ...
    return {
        'schema': 'cdeadmin.redis-object-gate.v1',
        'gate': 'redis-key-value-object-experience',
