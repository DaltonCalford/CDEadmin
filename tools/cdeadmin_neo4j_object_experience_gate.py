...
"""Gate Neo4j graph navigator and object-editor declarations."""
...
def provider_catalog(live_evidence_paths=()):
    adapter = Neo4jClient.__new__(Neo4jClient)
    catalog = enrich_engine_experience(
        adapter.visual_admin_catalog(catalog_for_engine('neo4j'))
    )
...
def audit(live_evidence_paths=()):
    ...
    return {
        'schema': 'cdeadmin.neo4j-object-gate.v1',
        'gate': 'neo4j-graph-object-experience',
