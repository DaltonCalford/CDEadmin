...
def normalize_http_route(request, *, default_port, default_auth='none',
    ...
    if result['auth_kind'] not in {'none', 'bearer', 'basic'}:
    ...
    if result.get('username') is not None:
        ...
        result.pop('username')
    if result['auth_kind'] == 'basic' and not result.get('username'):
        raise AnalyticHTTPError('basic authentication requires a username')
    if result['auth_kind'] != 'none':
        if not result.get('credential_reference_id'):
            raise AnalyticHTTPError(
                'authentication requires a credential reference'
            )
        if not result.get('principal_reference'):
            raise AnalyticHTTPError(
                'authentication requires a principal reference'
            )
    return result
...
class BoundedJSONHTTPTransport:
    ...
    def _with_secret(self, route, callback):
    ...
    def request(self, route, path, *, method='GET', query=None,
        ...
        def perform(secret):
            if secret is not None:
                if route['auth_kind'] == 'bearer':
                    request_headers['Authorization'] = f'Bearer {secret}'
                elif route['auth_kind'] == 'basic':
                    encoded = base64.b64encode(
                        f'{route["username"]}:{secret}'.encode('utf-8')
                    ).decode('ascii')
            ...
            try:
                response = opener(request, timeout, self._ssl_context(route))
                with response:
        ...
        return self._with_secret(route, perform)
