...
"""CQL/native-protocol boundary for Apache Cassandra 5.0.8.

The adapter owns Cassandra query, schema, consistency and topology semantics.
Every completion, timeout, lightweight-transaction result and retry remains an
opaque driver/server observation. Common CDEadmin code never interprets it as
...
class CassandraClient:
    ...
    ROUTE_KEYS = frozenset({
        ...
        'tool_workspace', 'jmx_port', 'tls_ca_file',
    })
    ...
    def _route(cls, request):
        ...
        if route.get('username') is not None and route.get(
            ...
            raise CassandraClientError(
                'Cassandra username requires a credential reference'
            )
        ...
        if route.get('tls_ca_file') is not None:
            ca_file = Path(_identifier(
                route['tls_ca_file'], 'Cassandra TLS CA file'
            ))
            if not ca_file.is_absolute():
                raise CassandraClientError(
                    'Cassandra TLS CA file must be absolute'
                )
            route['tls_ca_file'] = str(ca_file.resolve(strict=False))
    ...
    def _cluster_options(self, route, auth_provider):
        result = {
            'contact_points': route['contact_points'],
            'port': route['port'],
            'auth_provider': auth_provider,
            'ssl_context': self._ssl_context(route),
            'compression': (
                None if route['compression'] == 'none'
                else route['compression']
            ),
            'connect_timeout': float(route['connect_timeout']),
            'control_connection_timeout': float(route['connect_timeout']),
            'protocol_version': route['protocol_version'],
        }
        policy = getattr(self.module, 'DCAwareRoundRobinPolicy', None)
        if route.get('local_dc') and callable(policy):
            result['load_balancing_policy'] = policy(
                local_dc=route['local_dc']
            )
        return result
    ...
    def _connect(self, request):
        ...
        def connect(password=None):
            ...
            cluster = self.module.Cluster(
                **self._cluster_options(route, auth_provider)
            )
            try:
    ...
    def list_resources(self, request):
        ...
        try:
            ...
            for kind, statement in categories:
                try:
                    rows = self._rows(
                        session,
        ...
        local = local_rows[0] if local_rows else {'cluster_name': 'Cassandra'}
        cluster_name = local.get('cluster_name') or 'Cassandra'
        ...
        for kind, field_name in name_fields.items():
