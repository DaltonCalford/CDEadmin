...
from pgadmin.cdeadmin.sdk import ProviderToolRunner
...
class YugabyteDBYCQLClient(CassandraClient):
    ...
    VIRTUAL_RESOURCES = (
        ('query', 'YCQL query execution'),
        ('shell', 'YCQL native shell'),
    )
    ...
    def __init__(self, secret_acquirer=None, module=None,
        ...
        self._trace_receipts = {}
        self._tool_runner = ProviderToolRunner({
            'cqlsh': os.environ.get(
                'CDEADMIN_YUGABYTEDB_YCQLSH_BINARY', 'ycqlsh'
            ),
        })

    def execute(self, handle, request):
        """Execute YCQL and retain bounded native trace receipts."""
        try:
        ...
        if request.get('trace') is True:
            ...
            identity = str(
                trace_id or request.get('execution_id') or uuid.uuid4()
            )
            storage = (
                'rpc-response-receipt' if trace_id is not None
                else 'server-log'
            )
            self._trace_receipts[identity] = {
                'session_id': identity,
                ...
                'server_trace_storage': storage,
            }
            token.ycql_trace_receipt_id = identity
            token.ycql_trace_storage = storage
            if len(self._trace_receipts) > 1024:
                self._trace_receipts.pop(next(iter(
                    self._trace_receipts
                )))
        return token
    ...
    def describe_result(self, token):
        ...
        receipt_id = getattr(token, 'ycql_trace_receipt_id', None)
        if receipt_id is not None:
            observation = result['schema']['native_observation']
            observation['trace_requested'] = True
            observation['trace_request_receipt_id'] = receipt_id
            observation['trace_storage'] = token.ycql_trace_storage
        return result

    @classmethod
    def admin_operation_requires_dialect(cls, resource_kind, operation_id):
        """Identify only YCQL statements generated by visual tasks."""
        return (
    ...
    def admin_dialect_task_ids(cls):
        """Return the complete exact-profile YCQL task inventory."""
        return tuple(sorted(
    ...
    def list_resources(self, request):
        ...
        resources.extend(
            self._resource('tracing-session', trace_id, native)
            for trace_id, native in self._trace_receipts.items()
        )
        return [
    ...
    def visual_admin_catalog(self, catalog):
        ...
        def supported(*resource_kinds):
            return {
                ...
                'reason': (
                    'The exact YCQL provider owns these native objects and '
                    'operations.'
                ),
            }
        ...
        def shared_control(reason):
            return {
                'status': 'supported',
                'external_surface': 'cdeadmin.yugabytedb.control-plane',
        ...
        ycql_catalog.update({
            ...
            'concept_declarations': {
                'wide_column': {
                    'keyspaces': supported('keyspace'),
                    'tables': supported('table'),
                    'columns': supported('column'),
                    'types': supported('user-defined-type'),
                    'materialized_views': unavailable(
                        'YCQL does not expose Cassandra materialized views '
                        'in the exact provider profile.'
                    ),
                    'indexes': supported('index'),
                    'triggers': unavailable(
                        'YCQL has no Cassandra trigger object surface.'
                    ),
                    'functions': unavailable(
                        'YCQL does not expose Cassandra user-defined '
                        'functions.'
                    ),
                    ...
                    'roles_and_permissions': supported('role', 'permission'),
                    'identities': unavailable(
                        'YCQL does not implement Cassandra external identity '
                        'to role mappings.'
                    ),
                    'replication': shared_control(
                        'YugabyteDB owns replication at the shared cluster '
                        'control plane, not as a YCQL schema statement.'
                    ),
                    'compaction': shared_control(
                        'YugabyteDB owns tablet compaction at the shared '
                        'cluster control plane, not through nodetool.'
                    ),
                    'topology': supported(
                        'cluster', 'datacenter', 'node'
                    ),
                    'queries': supported('query'),
                    'tracing': supported('tracing-session'),
                    'repair': unavailable(
                        'YugabyteDB repairs tablet replicas automatically '
                        'and has no Cassandra repair operation.'
                    ),
                    'snapshots': shared_control(
                        'YugabyteDB snapshots are owned by the shared '
                        'cluster control plane.'
                    ),
                    'backup_and_restore': shared_control(
                        'YugabyteDB backup and restore are owned by the '
