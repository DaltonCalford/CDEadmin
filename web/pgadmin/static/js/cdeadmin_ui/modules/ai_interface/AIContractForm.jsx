/////////////////////////////////////////////////////////////
// Contract-driven renderer for every first-party AI Interface form.
/////////////////////////////////////////////////////////////
...
import {NumberField, SecretField, TextArea, TextField} from
  '../../primitives/Field';
import {ComboBox, DateField, MultiSelect, SegmentedControl,
  UnitNumberField} from '../../primitives/AdvancedControls';
...
import {evaluateAICondition, initialAIFormValues, isAIFormActionEnabled,
  redactedAIFormValues, transientAIFormSubmission,
  validateAIFormValues} from './AIFormRuntime';
...
function GridControl({field, value, columns}) {
  ...
  return <Box sx={{height: 240, minHeight: 160}}><DataGrid
    gridId={`ai-interface/form/${field.id}`} aria-label={field.label}
...
function FieldControl({field, value, onChange, options, gridColumns,
  resources, assets, connections, context, state, error}) {
  const disabled = DISABLED_STATES.has(state) ||
    !evaluateAICondition(field.enabled, context.values, context);
  ...
  default: throw new TypeError(`Unsupported AI form component: ${field.component}`);
  }
}
...
export function AIContractForm({form: formInput, initialValues=EMPTY_VALUES,
  ...
  const form = typeof formInput === 'string' ? AI_FORM_BY_ID[formInput] : formInput;
  if(!form) throw new TypeError('AIContractForm requires a known AI form contract.');
  const [values, setValues] = useState(() => initialAIFormValues(form, initialValues));
  const [submitted, setSubmitted] = useState(false);
  const [busyAction, setBusyAction] = useState('');
  const [runtimeError, setRuntimeError] = useState('');
  const [wizardStep, setWizardStep] = useState(0);
  useEffect(() => {
    setValues(initialAIFormValues(form, initialValues));
    setSubmitted(false); setRuntimeError(''); setWizardStep(0);
  }, [form, initialValues]);
  const runtimeContext = useMemo(() => ({...context, values}), [context, values]);
  const validation = useMemo(() => validateAIFormValues(
    form, values, runtimeContext), [form, values, runtimeContext]);
  const currentValidation = useMemo(() => form.kind === 'wizard' &&
    wizardStep < form.sections.length ? validateAIFormValues(
  ...
  const update = (fieldId, value) => setValues((current) => {
    ...
    onValuesChange?.(redactedAIFormValues(form, next));
    return next;
  });
  const invoke = async(action) => {
    ...
    if(!isAIFormActionEnabled(action, form, values, actionValidation,
  ...
  const actionEnabled = (action) => {
    ...
    if(form.kind === 'wizard' && ['test', 'create'].includes(action.id) &&
        wizardStep < form.sections.length) return false;
    return isAIFormActionEnabled(action, form, values,
  ...
  return <Box component="form" noValidate data-ai-form={form.form_id}
    ...
    <Box sx={{p: 1.5, overflow: 'auto', flex: 1, minHeight: 0}}>
      ...
      {visibleSections.map((section) => <FormSection key={section.id}
        ...
        <Box sx={{display: 'grid', gap: 1.5,
          ...
          {section.fields.filter((field) => evaluateAICondition(
            field.visibility, values, runtimeContext)).map((field) =>
            <Box key={field.id} data-ai-field={field.id}
              ...
              <FieldControl field={field} value={values[field.id]}
                ...
                state={state} error={(submitted || state === 'invalid') ?
      ...
      {form.kind === 'wizard' && wizardStep === form.sections.length &&
        <FormSection title="6 — Review"><Box component="dl" sx={{display: 'grid',
          gridTemplateColumns: 'minmax(140px, 1fr) 2fr', gap: 0.5, m: 0}}>
          {Object.entries(redactedAIFormValues(form, values)).map(([fieldId, value]) => <Box
